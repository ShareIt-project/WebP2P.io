...
(function(module) {
  ...
  if (window.FileWriter != undefined) return;

  Blob.slice = Blob.slice || Blob.webkitSlice || Blob.mozSlice;
  if (Blob.slice != undefined) alert("It won't work in your browser. Please use Chrome or Firefox.");
  ...
  FileWriter = function(blob) {
    if (!blob) throw Error('Expected blob argument to write.');
     ...
     * @return {Number} The position of the index inside the blob.
     */
     ...
     * @return {Number} The current lenght of the blob.
     */
    ...
    this.seek = function(offset) {
      ...
      if (position_ > this.length) position_ = this.length;
      else if (position_ < 0) position_ += this.length;
    ...
    this.truncate = function(size) {
      if (size < this.length) blob_ = blob_.slice(0, size);
      else blob_ = new Blob([blob_, new Uint8Array(size - this.length)], {
        'type': blob_.type
      });
    };
