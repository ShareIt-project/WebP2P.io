...
function WebP2P(e){function n(e){c.emit("error",new Error(e))}function o(e,n,o){var t=new RTCPeerConnection({iceServers:[{url:"stun:"+d}]},{optional:[{DtlsSrtpKeyAgreement:!0}]});return applyChannelsShim(t),t.addEventListener("icecandidate",function(e){if(!e.candidate){var t=this.localDescription.type,s=this.localDescription.sdp;t==n?o(s):console.error(t+" SDP type is not equal to "+n+" callback type")}}),t.addEventListener("signalingstatechange",function(){"stable"==t.signalingState&&(delete v[e],g.add(e,t))}),Object.defineProperty(t,"sessionID",{value:e}),t}function t(e,o){o=[c.routingLabel].concat(f,o);for(var t,s=0;t=o[s];s++)e.createDataChannel(t);var r={mandatory:{OfferToReceiveAudio:!1,OfferToReceiveVideo:!1}};e.createOffer(function(o){e.setLocalDescription(o,function(){console.log("["+c.sessionID+"] Offer LocalDescription created and set")},n)},n,r)}function s(e,o){e.setRemoteDescription(new RTCSessionDescription({sdp:o,type:"offer"}),function(){e.createAnswer(function(o){e.setLocalDescription(o,function(){console.log("["+c.sessionID+"] Answer LocalDescription created and set")},n)},n)},n)}function r(e,n){if(--e.ttl<=0)return console.warning("TTL achieved: "+e),void 0;var o=e.dest;e=e.pack();for(var t,s=0;t=g._connectors[s];s++)if(t.sessionID==o)return t.send(e);g.send(e,n),p.send(e,n)}function i(e){function t(e){var n=g.get(e);if(n)return console.log("["+c.sessionID+"] Already connected to "+e),!0;var o=v[e];if(o){if(console.log("["+c.sessionID+"] Already offered connection to "+e),c.sessionID<e)return!0;o.message.cancel(),o.peerConnection.close(),delete v[e]}return!1}e.on("connected",function(){p.status!=g.status&&c.emit("connected")}),e.on("disconnected",function(){p.status==g.status&&c.emit("disconnected")}),e.on("error",function(e){c.emit("error",e)}),e.on("offer",function(e,n){var i=e.from,a=e.dest;if(i!=c.sessionID)if(a==c.sessionID){if(console.log("["+c.sessionID+"] Received connection request from "+i),t(i))return;var f=e,u=o(i,"answer",function(e){var o=f.reply(i,e);n.send(o)});s(u,e.sdp)}else r(e,n)}),e.on("answer",function(e,o){var t=e.from,s=e.dest;if(t!=c.sessionID)if(s==c.sessionID){console.log("["+c.sessionID+"] Received connection response from "+t);var i=v[t];i?i.peerConnection.setRemoteDescription(new RTCSessionDescription({sdp:e.sdp,type:"answer"}),function(){console.log("Successfuly generated RemoteDescription for "+t)},n):console.warn("["+c.sessionID+"] Connection with peer '"+t+"' was not previously requested")}else r(e,o)})}var c=this,a=function h(e){return e?(e^16*Math.random()>>e/4).toString(16):([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g,h)},e=e||{},f=e.commonLabels||[],u=e.handshake_servers,d=e.stun_server||"stun.l.google.com:19302";Object.defineProperty(this,"routingLabel",{value:e.routingLabel||"webp2p"}),Object.defineProperty(this,"sessionID",{value:e.sessionID||a()});var l=new MessagePacker(this.sessionID),p=new HandshakeManager(l,u),g=new PeersManager(l,c.routingLabel);this.__defineGetter__("status",function(){return"connected"==g.status?"connected":p.status}),this.__defineGetter__("peers",function(){return g.peers});var v={};this.connectTo=function(e,n,s){if(n instanceof Function){if(s)throw new SyntaxError("Nothing can be defined after the callback");s=n,n=void 0}if(e==c.sessionID)return s(new Error("Connecting to ourself"));var r=g.get(e);r?s(null,r):(r=o(e,"offer",function(n){var o=l.offer(e,n,s);v[e]={message:o,peerConnection:r},p.send(o),g.send(o)}),t(r,n))},p.on("connected",function(){c.emit("handshakeManager.connected")}),p.on("disconnected",function(){c.emit("handshakeManager.disconnected")}),g.on("peerconnection",function(e){c.emit("peerconnection",e)}),i(p),i(g),p.on("presence",function(e,n){if(e!=c.sessionID){var s=g.get(e);if(!s){var r=o(e,"offer",function(o){var t=l.offer(e,o);v[e]={message:t,peerConnection:r},n.send(t)});r.addEventListener("signalingstatechange",function(){"stable"==r.signalingState&&n.increaseConnections()}),t(r)}}}),this.close=function(){p.close(),g.close()},window&&window.addEventListener("beforeunload",function(){c.close()})}var EventEmitter=require("events").EventEmitter,RTCPeerConnection=require("wrtc").RTCPeerConnection,HandshakeManager=require("./managers/HandshakeManager"),PeersManager=require("./managers/PeersManager"),MessagePacker=require("./MessagePacker"),applyChannelsShim=require("./PeerConnection_channels.shim");WebP2P.prototype.__proto__=EventEmitter.prototype,WebP2P.prototype.constructor=WebP2P,module.exports=WebP2P;
...
function Manager(e){EventEmitter.call(this);var n=this;this._connectors=[],this.__defineGetter__("status",function(){return this._connectors.length?"connected":"disconnected"}),this._initConnector=function(t){t.on("open",function(){n._connectors.push(t),1==n._connectors.length&&n.emit("connected")}),t.on("close",function(){n._connectors.splice(n._connectors.indexOf(t),1),0==n._connectors.length&&n.emit("disconnected")}),t.on("error",function(e){t.close(),n.emit("error",e)}),t._messageUnpacked=function(e){switch(e.type){case"offer":case"answer":n.emit(e.type,e,t);break;default:console.error("Unknown message type '"+type+"'"),console.error(e)}},t.on("message",function(n){n=e.unpack(n),n&&(n.stored?t.send(n):t._messageUnpacked(n))})},this.send=function(e,n){for(var t,o=0;t=this._connectors[o];o++)t!==n&&peer.send(e)},this.close=function(){for(var e,n=0;e=this._connectors[n];n++)e.close()}}var EventEmitter=require("events").EventEmitter;Manager.prototype.__proto__=EventEmitter.prototype,Manager.prototype.constructor=Manager,module.exports=Manager;
},{"events":13}],12:[function(require,module,exports){
function PeersManager(e,n){function t(e){var n=new Connector_DataChannel(e);return r._initConnector(n),n}Manager.call(this,e);var r=this,a={};this.__defineGetter__("peers",function(){return a}),this.add=function(e,o){function i(i){if(i.label==n){var s=t(i);return s.sessionID=e,a[e]=o,r.emit("peerconnection",o),!0}}function s(e){i(e.channel)&&o.removeEventListener("datachannel",s)}o.addEventListener("signalingstatechange",function(){"closed"==o.signalingState&&delete a[e]});var c=o.getDataChannels();if(c)for(var u,g=0;u=c[g];g++){if(i(u))return;o.addEventListener("datachannel",s)}},this.get=function(e){return a[e]}}var Manager=require("./Manager"),Connector_DataChannel=require("../connectors/core/DataChannel");PeersManager.prototype.__proto__=Manager.prototype,PeersManager.prototype.constructor=PeersManager,module.exports=PeersManager;
