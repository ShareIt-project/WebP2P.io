...
function HandshakeManager(n,e){function r(e){var r=e.type,o=e.config_init,s=e.config_mess,i=e.max_connections,c=a[r];if(!c)throw Error("Invalid handshake server type '"+r+"'");var d=new c(o,s,i);t._initConnector(d),d.on("open",function(){d.send(n.encode("presence"))});var u=d._messageUnpacked;return d._messageUnpacked=function(n){"presence"==n.method?d.shouldConnect()&&t.emit("presence",n.params.from,d):u.call(d,n)},d}function o(){if(!s.length)throw Error("No handshake servers defined");for(;i<s.length;i++){var n=r(s[i]);return void n.on("close",function(){i++,o()})}i=0}Manager.call(this,n);var t=this,a={};this.registerConnectorConstructor=function(n,e){a[n]=e},this.registerConnectorConstructor("PubNub",Connector_PubNub);var s=[],i=0,c=[];this.addConfigs_byObject=function(n){var e=a[n.type];return e?void(e.prototype.max_connections!=Number.POSITIVE_INFINITY||n.max_connections?(s.push(n),"disconnected"==t.status&&o()):(c.push(n),r(n))):console.error("Invalid handshake server config: ",n)},e&&this.addConfigs(e)}var inherits=_dereq_("inherits"),Manager=_dereq_("./Manager"),errors=_dereq_("../errors"),Connector_PubNub=_dereq_("../connectors/PubNub");inherits(HandshakeManager,Manager),HandshakeManager.prototype.addConfigs=function(n){return"string"==typeof n?this.addConfigs_byUri(n):n instanceof Array?this.addConfigs_byArray(n):void this.addConfigs_byObject(n)},HandshakeManager.prototype.addConfigs_byArray=function(n){for(var e,r=0;e=n[r];r++)this.addConfigs_byObject(e)},HandshakeManager.prototype.addConfigs_byUri=function(n){function e(n){r.emit("error",n)}var r=this,o=new XMLHttpRequest;o.open("GET",n),o.onload=function(){if(200==this.status){var n=JSON.parse(o.response);n.length?r.addConfigs_byArray(n):e(errors.ERROR_REQUEST_EMPTY)}else e(errors.ERROR_REQUEST_FAILURE)},o.onerror=function(){e(navigator.onLine?errors.ERROR_NETWORK_UNKNOWN:errors.ERROR_NETWORK_OFFLINE)},o.send()},module.exports=HandshakeManager;
...
function unifyResponseMethods(e){if(!e)return{};for(var r in e){var t=e[r];"string"==typeof t&&(e[r]={response:t})}return e}function checkValidTransport(e){if(e&&!(e instanceof Function||e.send instanceof Function))throw new SyntaxError("Transport is not a function nor have a send method")}function RpcNotification(e,r){Object.defineProperty(this,"method",{value:e,enumerable:!0}),Object.defineProperty(this,"params",{value:r,enumerable:!0})}function RpcBuilder(e,r,t){function n(e,r,t){var n={message:e,timeout:setTimeout(function(){h.remove(r,t)},p)};h.set(n,r,t)}function o(e,r){var t=setTimeout(function(){m.remove(e,r)},d);m.set(t,e,r)}function i(r,t,o,i,a){RpcNotification.call(this,r,t),this.transport=a;var u=h.get(o,i);Object.defineProperty(this,"duplicated",{value:Boolean(u)});var f=c[r];this.pack=function(){return e.pack(this,o)},this.reply=function(r,t,a){if(r instanceof Function||r&&r.send instanceof Function){if(void 0!=t)throw new SyntaxError("There can't be parameters after callback");a=r,t=null,r=void 0}else if(t instanceof Function||t&&t.send instanceof Function){if(void 0!=a)throw new SyntaxError("There can't be parameters after callback");a=t,t=null}checkValidTransport(a),u&&clearTimeout(u.timeout),void 0!=i&&(r&&(r.dest=i),t&&(t.dest=i));var c;if(r||void 0!=t){if(void 0!=s.peerID&&(r?r.from=s.peerID:t.from=s.peerID),f)if(void 0==f.error&&r)c={error:r};else{var p=r?f.error:f.response;c={method:p,params:r||t}}else c={error:r,result:t};c=e.pack(c,o)}else c=u?u.message:e.pack({result:null},o);return n(c,o,i),a=a||this.transport||s.transport,a?a instanceof Function?a(c):a.send(c):c}}function a(e){var r=w[e];if(r){delete w[e];var t=v.pop(r.id,r.dest);t&&(clearTimeout(t.timeout),o(r.id,r.dest))}}var s=this;if(!e)throw new SyntaxError("Packer is not defined");if(!e.pack||!e.unpack)throw new SyntaxError("Packer is invalid");var c=unifyResponseMethods(e.responseMethods);if(r instanceof Function||r&&r.send instanceof Function){if(t)throw new SyntaxError("There can't be parameters after callback");t=r,r=void 0}r=r||{},Object.defineProperty(this,"peerID",{value:r.peerID});var u=r.max_retries||0;checkValidTransport(t),this.transport=t;const f=r.request_timeout||BASE_TIMEOUT,p=r.response_timeout||BASE_TIMEOUT,d=r.duplicates_timeout||BASE_TIMEOUT;var l=0,v=new Mapper,h=new Mapper,m=new Mapper,w={};inherits(i,RpcNotification),this.cancel=function(e){if(e)return a(e);for(var e in w)a(e)},this.close=function(){t&&t.close&&t.close(),this.cancel(),m.forEach(function(e){clearTimeout(e)}),h.forEach(function(e){clearTimeout(e.timeout)})},this.encode=function(r,t,n,o,i){function a(e,r){s.cancel(T),i(e,r)}function p(e){return y.timeout=setTimeout(h,f*Math.pow(2,b++)),w[T]={id:E,dest:n},v.set(y,E,n),e=e||k||s.transport,e?e instanceof Function?e(T):e.send(T):T}function d(e){console.warn(b+" retry for request message:",T);var r=m.pop(E,n);return clearTimeout(r),p(e)}function h(){if(u>b)return d(o);var e=new Error("Request has timed out");e.request=T,e.retry=d,a(e)}if(t instanceof Function){if(void 0!=n)throw new SyntaxError("There can't be parameters after callback");i=t,o=void 0,n=void 0,t=void 0}else if(n instanceof Function){if(void 0!=o)throw new SyntaxError("There can't be parameters after callback");i=n,o=void 0,n=void 0}else if(o instanceof Function){if(void 0!=i)throw new SyntaxError("There can't be parameters after callback");i=o,o=void 0}var k=o;void 0!=s.peerID&&(t=t||{},t.from=s.peerID),void 0!=n&&(t.dest=n);var T={method:r,params:t};if(i){var E=l++,b=0;T=e.pack(T,E);var y={message:T,callback:a,responseMethods:c[r]||{}};return p(o)}return e.pack(T)},this.decode=function(r,t){function n(){checkValidTransport(t);var e=void 0!=u?u:f;return new i(p,d,e,l,t)}function a(e,r,t){e.callback(r,t)}function c(e){console.warn("Response already processed",r),clearTimeout(e),o(f,l)}if(!r)throw new TypeError("Message is not defined");r=e.unpack(r);var u=r.id,f=r.ack,p=r.method,d=r.params||{},l=d.from,h=d.dest;if(void 0==s.peerID||l!=s.peerID){if(void 0==u&&void 0==f)return new RpcNotification(p,d);if(p){if(void 0==h||h==s.peerID){var w=v.get(f,l);if(w){var k=w.responseMethods;return p==k.error?a(w,d):p==k.response?a(w,null,d):n()}var T=m.get(f,l);if(T)return c(T)}return n()}var E=r.error,b=r.result;if(!(E&&E.dest&&E.dest!=s.sessionID||b&&b.dest&&b.dest!=s.sessionID)){var w=v.get(f,l);if(!w){var T=m.get(f,l);return T?c(T):console.warn("No callback was defined for this message",r)}E&&(E=new RpcError(E,t)),a(w,E,b)}}}}var inherits=_dereq_("inherits"),packers=_dereq_("./packers"),Mapper=_dereq_("./Mapper");const BASE_TIMEOUT=5e3;RpcBuilder.RpcNotification=RpcNotification,module.exports=RpcBuilder,RpcBuilder.packers=packers;
