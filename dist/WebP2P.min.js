...
function WebP2P(e){function n(e){c.emit("error",new Error(e))}function o(e,n,o){var t=new RTCPeerConnection({iceServers:[{url:"stun:"+u}]},{optional:[{DtlsSrtpKeyAgreement:!0}]});return applyChannelsShim(t),t.addEventListener("icecandidate",function(e){if(!e.candidate){var t=this.localDescription.type,s=this.localDescription.sdp;t==n?o(s):console.error(t+" SDP type is not equal to "+n+" callback type")}}),t.addEventListener("signalingstatechange",function(){if("stable"==t.signalingState){p.add(e,t);var n=g[e];if(n){delete g[e];var o=n.callback;o&&o(null,t)}c.emit("peerconnection",t)}}),Object.defineProperty(t,"sessionID",{value:e}),t}function t(e,o){o=[c.routingLabel].concat(a,o);for(var t,s=0;t=o[s];s++)e.createDataChannel(t);var r={mandatory:{OfferToReceiveAudio:!1,OfferToReceiveVideo:!1}};e.createOffer(function(o){e.setLocalDescription(o,function(){console.log("["+c.sessionID+"] Offer LocalDescription created and set")},n)},n,r)}function s(e,o){e.setRemoteDescription(new RTCSessionDescription({sdp:o,type:"offer"}),function(){e.createAnswer(function(o){e.setLocalDescription(o,function(){console.log("["+c.sessionID+"] Answer LocalDescription created and set")},n)},n)},n)}function r(e,n){if(--e.ttl<=0)return console.warn("TTL achieved:",e);var o=e.dest;e=e.pack();for(var t,s=0;t=p._connectors[s];s++)if(t.sessionID==o)return t.send(e);p.send(e,n),l.send(e,n)}function i(e){function t(e){var n=p.get(e);if(n)return console.log("["+c.sessionID+"] Already connected to "+e),!0;var o=g[e];if(o){if(console.log("["+c.sessionID+"] Already offered connection to "+e),c.sessionID<e)return!0;o.message.cancel(),o.peerConnection.close(),delete g[e]}return!1}e.on("connected",function(){l.status!=p.status&&c.emit("connected")}),e.on("disconnected",function(){l.status==p.status&&c.emit("disconnected")}),e.on("error",function(e){c.emit("error",e)}),e.on("offer",function(e,n){var i=e.from,a=e.dest;if(i!=c.sessionID)if(a==c.sessionID){if(console.log("["+c.sessionID+"] Received connection request from "+i),t(i))return;var f=e,u=o(i,"answer",function(e){var o=f.reply(i,e);n.send(o)});s(u,e.sdp)}else r(e,n)}),e.on("answer",function(e,o){var t=e.from,s=e.dest;if(t!=c.sessionID)if(s==c.sessionID){console.log("["+c.sessionID+"] Received connection response from "+t);var i=g[t];i?i.peerConnection.setRemoteDescription(new RTCSessionDescription({sdp:e.sdp,type:"answer"}),function(){console.log("Successfuly generated RemoteDescription for "+t)},n):console.warn("["+c.sessionID+"] Connection with peer '"+t+"' was not previously requested")}else r(e,o)})}var c=this,e=e||{},a=e.commonLabels||[],f=e.handshake_servers,u=e.stun_server||"stun.l.google.com:19302";Object.defineProperty(this,"routingLabel",{value:e.routingLabel||"webp2p"}),Object.defineProperty(this,"sessionID",{value:e.sessionID||uuid.v4()});var d=new MessagePacker(this.sessionID),l=new HandshakeManager(d,f),p=new PeersManager(d,c.routingLabel);this.__defineGetter__("status",function(){return"connected"==p.status?"connected":l.status}),this.__defineGetter__("peers",function(){return p.peers});var g={};this.connectTo=function(e,n,s){if(n instanceof Function){if(s)throw new SyntaxError("Nothing can be defined after the callback");s=n,n=void 0}if(e==c.sessionID)return s(new Error("Connecting to ourself")),void 0;var r=p.get(e);r?s(null,r):(r=o(e,"offer",function(n){var o=d.offer(e,n,function(e,n){console.debug("["+c.sessionID+"] Received offer for connectTo from "+n.from),e&&s(e)});g[e]={callback:s,message:o,peerConnection:r},l.send(o),p.send(o)}),t(r,n))},l.on("connected",function(){c.emit("handshakeManager.connected",l)}),l.on("disconnected",function(){c.emit("handshakeManager.disconnected",l)}),p.on("connected",function(){c.emit("peersManager.connected",p)}),p.on("disconnected",function(){c.emit("peersManager.disconnected",p)}),i(l),i(p),l.on("presence",function(e,n){if(e!=c.sessionID){var s=p.get(e);if(!s){var r=o(e,"offer",function(o){var t=d.offer(e,o);g[e]={message:t,peerConnection:r},n.send(t)});r.addEventListener("signalingstatechange",function(){"stable"==r.signalingState&&n.increaseConnections()}),t(r)}}}),this.close=function(){l.close(),p.close()},window&&window.addEventListener("beforeunload",function(){c.close()})}var EventEmitter=require("events").EventEmitter,RTCPeerConnection=require("wrtc").RTCPeerConnection,uuid=require("uuid"),HandshakeManager=require("./managers/HandshakeManager"),PeersManager=require("./managers/PeersManager"),MessagePacker=require("./MessagePacker"),applyChannelsShim=require("./PeerConnection_channels.shim");WebP2P.prototype.__proto__=EventEmitter.prototype,WebP2P.prototype.constructor=WebP2P,module.exports=WebP2P;
...
function Manager(e){EventEmitter.call(this);var n=this;this._connectors=[],this.__defineGetter__("status",function(){return this._connectors.length?"connected":"disconnected"}),require("../connectors/core/DataChannel"),this._initConnector=function(t){t.on("open",function(){n._connectors.push(t),1==n._connectors.length&&n.emit("connected")}),t.on("close",function(){n._connectors.splice(n._connectors.indexOf(t),1),0==n._connectors.length&&n.emit("disconnected")}),t.on("error",function(e){t.close(),n.emit("error",e)}),t._messageUnpacked=function(e){switch(e.type){case"offer":case"answer":n.emit(e.type,e,t);break;default:console.error("Unknown message type '"+type+"'"),console.error(e)}},t.on("message",function(n){n=e.unpack(n),n&&(n.stored?t.send(n):t._messageUnpacked(n))})},this.send=function(e,n){for(var t,o=0;t=this._connectors[o];o++)t!==n&&t.send(e)},this.close=function(){for(var e,n=0;e=this._connectors[n];n++)e.close()}}var EventEmitter=require("events").EventEmitter;Manager.prototype.__proto__=EventEmitter.prototype,Manager.prototype.constructor=Manager,module.exports=Manager;
},{"../connectors/core/DataChannel":6,"events":13}],12:[function(require,module,exports){
function PeersManager(e,n){function t(e){var n=new Connector_DataChannel(e);return r._initConnector(n),n}function a(e,a){if(e.label==n){var r=t(e);return r.sessionID=a,r}}Manager.call(this,e);var r=this,o={};this.__defineGetter__("peers",function(){return o}),this.add=function(e,n){function t(n){var r=n.channel,o=a(r,e);o&&n.target.removeEventListener("datachannel",t)}n.addEventListener("signalingstatechange",function(){"closed"==n.signalingState&&delete o[e]});var r=n.getDataChannels();if(r.length)for(var i,s=0;(i=r[s])&&!a(i,e);s++);else n.addEventListener("datachannel",t);o[e]=n},this.get=function(e){return o[e]}}var Manager=require("./Manager"),Connector_DataChannel=require("../connectors/core/DataChannel");PeersManager.prototype.__proto__=Manager.prototype,PeersManager.prototype.constructor=PeersManager,module.exports=PeersManager;
