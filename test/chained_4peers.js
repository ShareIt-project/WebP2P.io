...
function()
{
  ...
  conn1.on('handshakeManager.connected', function()
  {
    ...
    conn2.on('peersManager.connected', function(_peersManager)
    {
      ...
      conn3.on('peerconnection', function(peerconnection)
      {
        ...
        {
          ok(false, 'Conn3 connected to Conn1');
      ...
      conn3.on('handshakeManager.disconnected', function()
      {
        ok(true, 'Conn3 handshakeManager.disconnected');
      });
      ...
      conn3.on('peersManager.connected', function(_peersManager)
      {
        ...
        {
          ok(false, "Conn4 error: "+error.message);
          console.error(error);
        ...
        conn4.on('handshakeManager.connected', function()
        {
          ok(true, "Conn4 handshakeManager.connected. SessionID: "+conn4.sessionID);
        });

        conn4.on('peerconnection', function(peerconnection)
        {
          ok(true, "Conn4 PeerConnection: "+peerconnection.sessionID);

          if(conn1.sessionID == peerconnection.sessionID)
          {
            ok(true, 'Conn4 connected to Conn1');

            var peers = Object.keys(conn4.peers);
            equal(peers.length, 2, "Conn4 peers: "+peers);
          ...
          if(conn2.sessionID == peerconnection.sessionID)
          {
            ok(false, 'Conn4 connected to Conn2');
        ...
        conn4.on('peersManager.connected', function(_peersManager)
        {
          ok(true, 'Conn4 peersManager.connected');

          var peers = Object.keys(conn4.peers);
          equal(peers.length, 1, "Conn4 peers: "+peers);

          var _connectors = _peersManager._connectors;
          equal(_connectors.length, 1, "Conn4 _connectors: "+_connectors.length);
          console.log(_connectors);


          conn4.connectTo(conn1.sessionID, function(error, peer)
          {
            if(error) return ok(false, error);

            equal(conn1.sessionID, peer.sessionID,
                  'Conn4 connected to Conn1: '+peer.sessionID);
  ...
  conn1.on('peerconnection', function(peerconnection)
  {
    ...
    if(conn4 && conn4.sessionID == peerconnection.sessionID)
    {
      ok(true, 'Conn1 connected to Conn4');
        ...
        conn4.on('peersManager.connected', function(_peersManager)
        {
          ...
          conn4.connectTo(conn1.sessionID, function(error, peerconnection)
          {
            ...
            equal(conn1.sessionID, peerconnection.sessionID,
                  'Conn4 connected to Conn1: '+peerconnection.sessionID);
